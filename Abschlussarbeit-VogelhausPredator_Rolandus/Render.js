...
(function (Abschlussarbeit) {
    class Render {
        ...
        static hill(_crc2, _x, _y, _sizeX, _sizeY, _color, _snowColor) {
            _crc2.transform(1, 0, 0, 1, _x, _y - _sizeY);
            let gradient = _crc2.createLinearGradient(0, 0, 0, _sizeY);
        ...
        static ground(_crc2) {
            let width = _crc2.canvas.width;
            let height = _crc2.canvas.height;
            let gradient = _crc2.createLinearGradient(0, height / 2.7, 0, height);
        ...
        static trees(_crc2, _treeArray) {
            for (let tree of _treeArray) {
                tree.draw(_crc2);
            }
        }
        static birdsHouse(_crc2) {
            let width = _crc2.canvas.width;
            let height = _crc2.canvas.height;
            let houseWidth = width / 20;
            let totalHeight = height / 3.2;
            let houseHeight = totalHeight / 3;
        ...
        static birds(_crc2, _birdArray) {
            for (let bird of _birdArray) {
                bird.draw(_crc2);
            }
        }
        static snow(_crc2, _snowArray) {
            for (let snow of _snowArray) {
                snow.draw(_crc2);
            }
        }
        static seeds(_crc2, _feedingArray) {
            for (let i = _feedingArray.length; i > 0; i--) {
                for (let seed of _feedingArray[i - 1]) {
                    seed.draw(_crc2);
                }
            }
        }
        static snowball(_crc2, _snowballs) {
            for (let ball of _snowballs) {
                ball.draw(_crc2);
            }
        }
    }
    Abschlussarbeit.Render = Render;
